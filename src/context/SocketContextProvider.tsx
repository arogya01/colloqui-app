...
export function SocketContextProvider({
  ...
  const reconnecWithBackOff = () => {
    if (reconnectAttempt >= maxReconnectAttempts) {
      console.log("Max Reconnection reached");
      return;
    }
  ...
  const connectWebSocket = useCallback(() => {
    ...
    const newWs = new WebSocket(
      "wss://dumdum12.azurewebsites.net/api/colloqui/chat?id=19",
      session
    );
    wsRef.current = newWs;
    setWs(newWs);
    ...
    newWs.onmessage = (event) => {
      ...
      if (response.type === CHAT_EVENTS.MESSAGE_SENT) {
        ...
        const { data: { createdMessage: { conversationId = "" } = {} } = {} } =
          response || {};
        fetchMessages(conversationId);
      }
  ...
  const createMessage = ({
    ...
    console.log("createMessage", { conversationId, message, senderId });
    if (wsRef.current?.readyState === WebSocket.OPEN) {
      wsRef.current.send(
        JSON.stringify({
